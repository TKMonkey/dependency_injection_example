...
import { HttpRequester } from "../../../di/3_with_logging_decorator/http_requester_implementations/http_requester";
...
import { LargeProcessing } from "../../../di/3_with_logging_decorator/large_processing";
...
describe("DIProcessing", () => {
  it("should request user info with user url", async () => {
    ...
    const diProcessing = new LargeProcessing(
      fakeHttpRequester,
      fakeDatabaseProxy
    );
    // Act
...
describe("HttpRequester", function () {
  ...
  it("should log http requests with user url", async () => {
    ...
    const diProcessing = new LargeProcessing(httpRequester, fakeDatabaseProxy);
    // Act
    await diProcessing.handleUserRequest("userId");
